 ...
 *
 * @param {string} sql    - consulta que retorna uma coluna numérica "total"
 * @param {Array}  params - parâmetros da consulta (opcional)
 * @param {string} origem - tabela/módulo de origem, para rastreabilidade
 * @param {object} [opts] - { zeroComoAguardando: boolean }
 *        Quando true, um total = 0 vira "aguardando_dados" em vez de "real".
 */
 ...
 * Se a tabela não tem lançamentos, o KPI vem como "aguardando_dados".
 *
 * @param {string} sql    - consulta que retorna "total" (SUM) e "qtd" (COUNT)
 * @param {Array}  params - parâmetros da consulta
 * @param {string} origem - tabela/módulo de origem
 */
async function somar(sql, params, origem) {
  try {
    ...
    // Sem nenhum lançamento no período -> ainda não há base financeira.
    if (qtd === 0) {
      return kpi(0, 'aguardando_dados', origem);
    }
    return kpi(total, 'real', origem);
...
const metricsService = {
   ...
   * Nota: hoje a base pode não ter usuários com tipo 'lider' -> retorna 0 real.
   */
  lideres() {
    return contar(
   ...
   * Dependências: receitaDoMes(), despesaDoMes().
   */
  async saldoDoMes() {
    ...
    // Se qualquer consulta falhou, propaga erro.
    if (receita.estado === 'erro' || despesa.estado === 'erro') {
      return kpi(null, 'erro', 'financeiro');
    }

    // Só é "real" se houve algum lançamento (receita ou despesa).
   ...
   * Executa em paralelo. Cada chave mantém o contrato padronizado.
   */
