...
router.post('/', autenticar, async (req, res) => {
  const { turma_id, nome_completo, telefone, email, observacoes } = req.body;
  if (!turma_id || !nome_completo) {
    return res.status(400).json({ erro: 'turma_id e nome_completo sao obrigatorios' });
  }
  try {
    const resultado = await pool.query(
      `INSERT INTO inscricoes_cursos (turma_id, usuario_id, nome_completo, telefone, email, observacoes)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [turma_id, req.usuario.id, nome_completo, telefone || null, email || null, observacoes || null]
    );
...
router.put('/:id', autenticar, somenteAdmin, async (req, res) => {
  const { nome_completo, telefone, email, status, observacoes } = req.body;
  try {
    const resultado = await pool.query(
      `UPDATE inscricoes_cursos SET
       ...
       WHERE id = $6
       RETURNING *`,
      [nome_completo, telefone, email, status, observacoes, req.params.id]
    );
    if (resultado.rows.length === 0) {
      return res.status(404).json({ erro: 'Inscricao nao encontrada' });
    }
